...
#
# Importer of cim files/directories into OrientDB
#
    ...
    def expand name, scheme, &block
      ...
      unless try "", name # name is not a reachable path
        ...
        @includes.unshift(".").each do |inc|
          # test <name>, <name>.cim, CIM_<name>.cim
          ...
          unless name =~ /^.*\.cim$/
            break if try inc, "#{name}.cim"
            break if scheme && try(inc, File.join(scheme,"#{name}.cim"))
          end
          unless name =~ /^(CIM_.*|.*\/.*)$/
            break if try inc, "CIM_#{name}.cim"
            break if scheme && try(inc, File.join(scheme,"CIM_#{name}.cim"))
          end
        end
      end
      ...
      when :file
        ...
        STDERR.puts "Can't find cim at #{name}"
        raise Errno::ENOENT
      end
    end
